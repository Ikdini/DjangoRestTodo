...
class TodoAPITestCase(APITestCase):
  ...
  def authenticate(self):
    ...
    response = self.client.post(
        reverse('login'), {'email': 'email@example.com', 'password': 'password'})
...
class TodoDetailAPIView(TodoAPITestCase):
  ...
  def test_updates_one_item(self):
    ...
    res = self.client.patch(
        reverse('todo', kwargs={'id': response.data['id']}), {"title": "New", "is_complete": True})
    ...
    self.assertEqual(updated_todo.title, 'New')
